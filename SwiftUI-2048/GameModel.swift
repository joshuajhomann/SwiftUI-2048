...
final class GameModel: ObservableObject {
  ...
  func shift(direction: SwipeGesture.Direction) {
  ...
  private let directionSubject = PassthroughSubject<SwipeGesture.Direction, Never>()
  ...
  init() {
    directionSubject
      ...
      .map { [weak self] direction -> ([Int: Cell], [Cell]) in
        ...
        if let index = (0..<Constant.dimension * Constant.dimension)
          .filter({ cells[$0] == nil })
          .randomElement() {
          cells[index] =  Cell(index: index, value: cells.isEmpty ? 2 : Int.random(in: 1...2) * 2)
        }
        return (cells, Array(cells.values))
      }
